...
def load_gptj(cache_dir=None):
    device = get_device()
    tokenizer = AutoTokenizer.from_pretrained("EleutherAI/gpt-j-6B", cache_dir=cache_dir)
    model = AutoModelForCausalLM.from_pretrained("EleutherAI/gpt-j-6B", revision='float16', torch_dtype=torch.float16, cache_dir=cache_dir).to(device)
...
class ModelWrapper(nn.Module):
    ...
    def tokenize_with_string(self, text):
    ...
    def get_top_ids_per_layer(self, logits, k=10):
        # TODO: make a tensor of strings
        ...
        for i, layer in enumerate(logits):
            ...
            # layer_tokens.append(self.tokenizer.convert_ids_to_tokens(F.softmax(layer,dim=-1).argsort(descending=True)[:k]))
        return layer_tokens

    def print_top(self, logits, k=10):
        result_output = ""
        for i,layer in enumerate(logits):
            result_output += f"{i} {'|'.join(self.tokenizer.convert_ids_to_tokens(F.softmax(layer,dim=-1).argsort(descending=True)[:k]))}\n"
        print(result_output)
    ...
    def print_top_concise(self, logits):
        ...
        for i,layer in enumerate(logits):
            # TODO: add k!=1
            top_token = self.tokenizer.convert_ids_to_tokens(F.softmax(layer,dim=-1).argsort(descending=True)[:1])
    ...
    def print_tops_for_text(self, text, translated="", offset=None, concise=False, markdown=False, k=3):
        ...
        if offset:
            ...
            logits = self.get_layers(inp_ids)
            result_output = self.print_top(logits, k=k)
        return result_output
