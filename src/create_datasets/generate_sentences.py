...
def format_sentence(df_row, format, lang, np_sub="Ich", np_sub_eng="I"):
    verb_perf = df_row.perf_ger
    verb_eng = df_row.verb_eng
    verb_past_eng = df_row.past_eng
    if ' ' in verb_eng:
        return None
    noun_obj = df_row.noun_ger
    noun_obj_eng = df_row.noun_eng
    ...
    if np_sub in pronoun2haben.keys():
        ...
        haben = 'hat'
    if lang == 'en':
        return format.format(np_sub=np_sub_eng, verb_past=verb_past_eng, noun_obj=noun_obj_eng)
    elif lang == 'ge':
        return format.format(np_sub=np_sub, haben=haben,article_acc=article_acc,noun_obj=noun_obj,verb_perf=verb_perf)

# TODO: separate formats
def generate_sentences(df, format_eng, format_ger):
    sentences_eng = []
    sentences_ger = []
    for i, row in df.iterrows():
        ...
        for np_sub, np_sub_eng in np2eng_nom.items():
            ...
            sentence_ger = format_ger.format(np_sub=np_sub, haben=haben,article_acc=article_acc,noun_obj=noun_obj,verb_perf=verb_perf)
            sentence_eng = format_eng.format(np_sub=np_sub_eng, verb_past=verb_past_eng, noun_obj=noun_obj_eng)

            sentences_ger.append(sentence_ger)
            sentences_eng.append(sentence_eng)

    return sentences_eng, sentences_ger
...
if __name__ == "__main__":
    for i, row in verbnoun_df.iterrows():
        ...
        for np_sub, np_sub_eng in np2eng_nom.items():
            ...
            sentence_dict['impf_3'].append(row.impf_3)
